...
把测试用例一次写完整（写完不运行，运行留给第 3 步）：覆盖 set/get、缺键、has、clear。
...
把测试用例一次写完整（写完不运行，运行留给第 3 步）：断言第二次调用命中缓存、TTL 到期后重新取数、不同实参不共享条目。
...
把测试用例一次写完整（写完不运行，运行留给第 3 步）：断言过期条目在下次访问时被淘汰。
